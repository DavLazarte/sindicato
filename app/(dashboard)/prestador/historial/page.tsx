...
export default function PrestadorHistorial() {
  ...
  const fetchData = useCallback((p: number) => {
    setLoading(true);
    api.get<ApiResponse<PaginatedData<Transaccion>>>(`/prestador/transacciones?page=${p}${search ? `&search=${search}` : ''}`)
      ...
      <div className="p-4 md:p-6">
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="w-full sm:w-96 relative">
              ...
              <input 
                ...
                placeholder="Buscar por nombre o legajo de socio..." 
          ...
          {loading ? (
            ...
            <div className="p-12 text-center"><span className="text-4xl block mb-3">📦</span><p className="text-slate-400 text-sm">Sin cobros registrados</p></div>
          ) : (
                ...
                return (
                  <div key={t.id} className="flex items-center gap-4 p-4 hover:bg-slate-50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">{t.socio?.legajo} · {formatDate(t.created_at)}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className={`text-sm font-bold ${isAnulada ? 'text-slate-400 line-through' : 'text-emerald-600'}`}>
                        {displayAmount}
                      </p>
